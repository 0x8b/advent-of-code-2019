...
def track(wire):
    wire = ((d[0], int(d[1:])) for d in wire.split(","))
...
route_one = track(wires[0])
route_two = track(wires[1])

common_points = set(route_one.keys()).intersection(set(route_two.keys()))

m = min(common_points, key=lambda x: manhattan(x))

part_one = int(manhattan(m))
assert part_one == 260

part_two = sum(
    min(
        ((route_one[p], route_two[p]) for p in common_points), key=lambda p: p[0] + p[1]
    )
)
assert part_two == 15612
